...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-mr-benn runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-mr-benn.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_mr_benn")
...
def lint(c):
    ...
    """
    c.run("flake8 django-mr-benn tests")
...
def release(c, bumpsize=''):
    ...
    import django_mr_benn
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_mr_benn.__version__))
    c.run("git push --tags")
